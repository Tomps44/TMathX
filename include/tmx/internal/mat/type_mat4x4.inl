namespace tmx
{
    ...
    template<typename T>
    TMX_INLINE constexpr typename mat<4, 4, T>::colType& mat<4, 4, T>::operator[](int i) 
    {
        return columns[i];
    }
    template<typename T>
    TMX_INLINE constexpr const typename mat<4, 4, T>::colType& mat<4, 4, T>::operator[](int i) const
    {
        return columns[i];
    }
