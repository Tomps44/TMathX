namespace tmx
{
    ...
    namespace Mat
    {
        ...
        {
            return internal::matInverse<R, C, T, internal::useSimd<R, T>::value>::call(m);
        }

        template<int R, int C, typename T>
        TMX_INLINE constexpr mat<R, C, T> AffineInverse(const mat<R, C, T>& m) noexcept
        {
            return internal::matAffineInverse<R, C, T, internal::useSimd<R, T>::value>::call(m);
        }

        template<int R, int C, typename T>
        TMX_INLINE constexpr mat<R, C, T> InverseTranspose(const mat<R, C, T>& m) noexcept
        {
            return internal::matInverseTranspose<R, C, T, internal::useSimd<R, T>::value>::call(m);
        }
